...
def years_gold_value_decreased(gold_prices: str = gold_prices) -> (int, int):
    ...
    first_year = min(prices.keys())
    last_year = max(prices.keys())

    changes = {}
    for year in range(first_year + 1, last_year + 1):
        changes[year] = prices[year] - prices[year - 1]

    changes_sorted = sorted(changes.items(), key=lambda x: x[1])

    return changes_sorted[0][0], changes_sorted[-1][0]
