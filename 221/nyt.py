...
YOUR_KEY = 'txAzrRYCbue71DqEoNAaOOi8zeDagLmC'
...
def get_best_seller_titles(url=URL_NON_FICTION):
    ...
    json_ = resp.json()
    books = json_["results"]["books"]
    book_weeks = [(book["title"], book["weeks_on_list"]) for book in books]
    return sorted(book_weeks, key=lambda x: x[1], reverse=True)
